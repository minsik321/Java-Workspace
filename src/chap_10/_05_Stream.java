...
public class _05_Stream {
    public static void main(String[] args) {
        ...
        // 4글자 이하의 언어를 정렬해서 출력
        langList.stream().filter(x -> 4 >= x.length()).sorted().forEach(System.out::println);
        System.out.println("----------------");
        ...
        // 3글자 이하의 언어들은 모두 c 라는 글자를 포함 하는지 여부
        ...
        // map (사용하는 데이터를 원하는 형태로 가공하거나 아니면 그 데이터(객체) 중에서 꺼내고싶은 변수를 지정할 수 있음)
        langList.stream()
                .filter(x -> 4 >= x.length())
                .filter(x -> x.contains("c"))
                .map(x -> x + " (어려워요)")
